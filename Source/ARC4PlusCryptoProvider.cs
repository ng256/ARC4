...
#endif
    internal unsafe class ARC4PlusCryptoProvider : CryptoProvider, IDisposable, ICloneable
    {
        ...
        [MethodImpl(MethodImplOptions.Synchronized)]
        public ARC4PlusCryptoProvider(byte[] iv, int index = 0)
        {
            if (iv == null)
        ...
        public ARC4PlusCryptoProvider(int seed) : this(BitConverter.GetBytes(seed))
        {
        }

        public ARC4PlusCryptoProvider(uint seed) : this(BitConverter.GetBytes(seed))
        {
        }
        ...
        private void Initialize(byte[] key)
        {
            ...
            for (int i = 0, j = 0, k = 0, l = 0; i < m; i++)
            {
                ...
                Swap(_sblock, i, j);
                k = (k + _sblock[i] + _sblock[j]) & (m - 1);
                Swap(_sblock, i, k);
                l = (l + _sblock[i] + _sblock[k]) & (m - 1);
                Swap(_sblock, i, l);
            }
        }
        ...
        private byte GeneratePRGA()
        {
            ...
            byte c = (byte)((_sblock[i] + _sblock[j]) & f);
            byte output = (byte)((_sblock[(a + b) & f] + _sblock[(c ^ r) & f]) ^ _sblock[(_y + b) & f]);
        ...
        public override object Clone()
        {
            ...
            ARC4PlusCryptoProvider clone = (ARC4PlusCryptoProvider)this.MemberwiseClone();
